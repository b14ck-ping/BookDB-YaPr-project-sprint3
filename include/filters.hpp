...
inline auto YearBetween(int YearFrom, int YearTo) noexcept {
    return [YearFrom, YearTo](const Book &book) { return book._year >= YearFrom && book._year <= YearTo; };
}

inline auto RatingAbove(double rating) noexcept {
    return [rating](const Book &book) { return book._rating >= rating; };
}

inline auto GenreIs(Genre genre) noexcept {
    return [genre](const Book &book) { return book._genre == genre; };
}

inline auto AuthorIs(std::string author) noexcept {
    return [author](const Book &book) { return book._author == author; };
}
...
auto any_of(Predicates... preds) noexcept {
    return [... preds = std::move(preds)](const Book &book) -> bool { return (preds(book) || ...); };
}

template <BookIterator Iterator, BookPredicate Predicate>
std::vector<std::reference_wrapper<const Book>> filterBooks(Iterator first, Iterator last, Predicate &&pred) noexcept {
    std::vector<std::reference_wrapper<const Book>> out;
    std::copy_if(first, last, std::back_inserter(out),
                 [pred = std::forward<Predicate>(pred)](const Book &book) { return pred(book); });
